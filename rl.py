...
'''
        ...
        # A state is a multi-dimensional array holding image-frames from
        # the game-environment.
            self.x =torch.FloatTensor(np.zeros(state_shape)[None, :, :])
        #tf.placeholder(dtype=tf.float32, shape=[None] + state_shape, name='x')

        # Placeholder variable for inputting the learning-rate to the optimizer.
            self.learning_rate = torch.FloatTensor()#tf.placeholder(dtype=tf.float32, shape=[])

        # Placeholder variable for inputting the target Q-values
        # that we want the Neural Network to be able to estimate.
            print(num_actions)
            #print(self.count_states+"CS")

            self.q_values_new = torch.IntTensor(1)
        ...
        # We will keep it as a variable in the TensorFlow-graph
        # even though it will not actually be used by TensorFlow.
            self.count_states =torch.IntTensor(1)# tf.Variable(initial_value=0,
                                    #trainable=False, dtype=tf.int64,
                                        #name='count_states')

        # Similarly, this is the counter for the number of episodes.
            self.count_episodes = Variable(torch.IntTensor(1))#tf.Variable(initial_value=0,
                                            #trainable=False, dtype=tf.int64,
                                            #name='count_episodes')
            ...
            self.conv2 = nn.Conv2d(6, 16, kernel_size=3,stride=2)
